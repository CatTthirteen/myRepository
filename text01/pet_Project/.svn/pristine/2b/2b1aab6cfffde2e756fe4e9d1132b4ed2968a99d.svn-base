...
//        layer_show("菜单树","application/menu/menuTree.jsp",300,400);
    ...
    htmlStr += '<div>' +
        '<table><tr><td><input type="hidden" id="parentMentId" width="50px"/>' +
        '<input type="text" id="parentMenuName" width="50px">' +
        '<button onclick="selectParentMenu();">确定</button></td></tr></table>' +
        '</div>';
...
function initZtree() {
    ...
    var setting = {
        async: {//异步方式调用
            enable: true,
            url: "menu/getMenuListByParentId",
...
function selectParentMenu() {
    ...
    //1.先要把用户选择的菜单获取到
    var pname= $("#parentMenuName").val();
    var pid=  $("#parentMentId").val();

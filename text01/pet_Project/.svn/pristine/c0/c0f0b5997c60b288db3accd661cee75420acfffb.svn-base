...
<div class="pd-20">
    ...
    <%--<form method="post" action="product/getDiscussImage">--%>
        ...
        <div class="row cl">
            <div class="cl pd-5">
                <button type="submit" onclick="location.href='product/getDiscussImage/${id}'"
                        class="btn btn-success radius" id="" name="">
                    <i class="Hui-iconfont">&#xe665;</i> 只看有图片评价
                </button>
                <button type="button"
                        onclick="location.href='product/getDiscuss/${id}'"
    ...
    <table class="table table-border table-bordered table-bg">
       <tr>
        ...
        <c:forEach items="${page.records}" var="discuss">
            <tr class="text-c">
                <td>${discuss.name}</td>
                <td>${discuss.pname}</td>
                <td>${discuss.content}</td>
                <td><c:if test="${not empty discuss.image}"><img src="${discuss.image}" width="70px" height="70px"></c:if></td>
            </tr>
        </c:forEach>
    </table>
